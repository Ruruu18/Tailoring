<x-app-layout>
    <x-slot name="header">
        <div class="space-y-2">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Add Your Measurements</h1>
                <p class="text-sm text-gray-600">Choose a standard size or enter custom measurements.</p>
            </div>
        </div>
    </x-slot>

    <div class="py-4">
        <div class="max-w-4xl mx-auto px-4 sm:px-3 lg:px-8">
            <form action="{{ route('measurements.store') }}" method="POST" id="measurementForm">
                @csrf

                <!-- Mode Selection -->
                <div class="bg-white rounded-lg shadow-md mb-3">
                    <div class="p-3">
                        <h2 class="text-md font-semibold text-gray-900 mb-3">How would you like to add measurements?</h2>
                        <div class="flex flex-col sm:flex-row gap-2">
                            <button type="button" id="standardSizeBtn" class="size-mode-btn active flex-1 px-3 py-2 rounded border-2 border-blue-500 bg-blue-500 text-white font-medium text-sm">
                                Standard Sizes
                            </button>
                            <button type="button" id="customSizeBtn" class="size-mode-btn flex-1 px-3 py-2 rounded border-2 border-gray-300 bg-white text-gray-700 font-medium text-sm">
                                Custom Measurements
                            </button>
                        </div>
                    </div>
                ...
                <div id="standardSizeSection" class="bg-white rounded-lg shadow-md">
                    <div class="p-3">
                        <!-- T-Shirt Visual -->
                        ...
                        <div class="flex flex-wrap justify-center gap-1 mb-3">
                            ...
                            @foreach($sizes as $size)
                                <button type="button" class="size-btn w-10 h-10 rounded border-2 border-gray-300 bg-white text-gray-700 font-bold text-xs" data-size="{{ $size }}">
                                    {{ $size }}
                                </button>
                            @endforeach
                        </div>
                        ...
                        <!-- Selected Size Info -->
                        <div id="sizeInfo" class="hidden bg-blue-50 border border-blue-200 rounded p-2 mb-3">
                            <h4 class="text-xs font-semibold text-blue-900 mb-1 text-center">Size <span id="selectedSizeName" class="bg-blue-500 text-white px-1 py-0.5 rounded text-xs"></span></h4>
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="1" class="w-full border border-gray-300 rounded px-2 py-1 text-xs" placeholder="Any notes..."></textarea>
                        </div>

                        <!-- Hidden inputs for standard size -->
                        <input type="hidden" name="size" id="selectedSize">
                        <input type="hidden" name="is_custom" value="0">
                        <input type="hidden" name="chest" id="standardChest">
                        <input type="hidden" name="waist" id="standardWaist">
                        <input type="hidden" name="hip" id="standardHip">
                        <input type="hidden" name="shoulder" id="standardShoulder">
                        <input type="hidden" name="sleeve_length" id="standardSleeve">
                        <input type="hidden" name="shirt_length" id="standardShirtLength">
                        <input type="hidden" name="short_waist" id="standardShortWaist">
                        <input type="hidden" name="inseam" id="standardInseam">
                    </div>
                </div>

                <!-- Custom Measurements Section -->
                <div id="customSizeSection" class="hidden bg-white rounded-lg shadow-md">
                    <div class="p-3">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <div>
                                ...
                                <input type="number" name="custom_chest" step="0.1" min="0" max="200" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="40.5">
                            </div>

                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Waist *</label>
                                <input type="number" name="custom_waist" step="0.1" min="0" max="200" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="32.0">
                            </div>

                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Hip *</label>
                                <input type="number" name="custom_hip" step="0.1" min="0" max="200" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="38.0">
                            </div>

                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Shoulder *</label>
                                <input type="number" name="custom_shoulder" step="0.1" min="0" max="100" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="18.0">
                            </div>

                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Sleeve *</label>
                                <input type="number" name="custom_sleeve_length" step="0.1" min="0" max="100" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="25.0">
                            </div>

                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Inseam *</label>
                                <input type="number" name="custom_inseam" step="0.1" min="0" max="150" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="32.0">
                            </div>

                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Short Waist</label>
                                <input type="number" name="custom_short_waist" step="0.1" min="0" max="200" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="30.0">
                            </div>

                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Shirt Length</label>
                                <input type="number" name="custom_shirt_length" step="0.1" min="0" max="100" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="28.0">
                            </div>
                        </div>
                        ...
                        <div class="mt-2">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Notes</label>
                            <textarea name="custom_notes" rows="1" class="w-full border border-gray-300 rounded px-1 py-1 text-xs" placeholder="Any notes..."></textarea>
                        </div>
                ...
                <div class="mt-3 flex gap-2">
                    <button type="submit" id="submitBtn" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm font-medium" disabled>
                        Save Measurements
                    </button>
