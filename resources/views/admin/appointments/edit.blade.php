<x-admin-layout>
    <x-slot name="header">
        <div class="space-y-2">
            <div>
                <a href="{{ route('admin.appointments.show', $appointment) }}" class="inline-flex items-center text-sm text-gray-600 hover:text-gray-800 transition-colors duration-200">
            ...
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Update appointment details for {{ $appointment->user->name }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <form action="{{ route('admin.appointments.update', $appointment) }}" method="POST" space-y-6>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="order_id" id="order_id" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($orders as $order)
                                        <option value="{{ $order->id }}" {{ (old('order_id', $appointment->order_id) == $order->id) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="service_type" id="service_type" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    <option value="fitting" {{ (old('service_type', $appointment->service_type) == 'fitting') ? 'selected' : '' }}>Fitting</option>
                                    <option value="measurement" {{ (old('service_type', $appointment->service_type) == 'measurement') ? 'selected' : '' }}>Measurement</option>
                                    <option value="pickup" {{ (old('service_type', $appointment->service_type) == 'pickup') ? 'selected' : '' }}>Pickup</option>
                                    <option value="delivery" {{ (old('service_type', $appointment->service_type) == 'delivery') ? 'selected' : '' }}>Delivery</option>
                                </select>
                                @error('service_type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
