<x-admin-layout>
    ...
    <div class="py-4">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.measurements.update', $measurement) }}" method="POST" id="measurementForm">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-lg shadow-md mb-3">
                    <div class="p-3">
                        ...
                        <div class="grid grid-cols-1 gap-2">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Select Customer *</label>
                                <select name="user_id" required class="w-full border border-gray-300 rounded px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                    <option value="">Choose a customer...</option>
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ $measurement->user_id == $user->id ? 'selected' : '' }}>
                ...
                <div class="bg-white rounded-lg shadow-md mb-3">
                    <div class="p-3">
                        <h3 class="text-md font-semibold text-gray-900 mb-3">Measurement Type</h3>
                        <div class="flex flex-col sm:flex-row gap-2">
                            <button type="button" id="standardSizeBtn" class="size-mode-btn {{ !$measurement->is_custom ? 'active' : '' }} flex-1 px-3 py-2 rounded-lg border-2 {{ !$measurement->is_custom ? 'border-blue-500 bg-blue-500 text-white' : 'border-gray-300 bg-white text-gray-700' }} font-medium text-sm">
                                Standard Sizes
                            </button>
                            <button type="button" id="customSizeBtn" class="size-mode-btn {{ $measurement->is_custom ? 'active' : '' }} flex-1 px-3 py-2 rounded-lg border-2 {{ $measurement->is_custom ? 'border-blue-500 bg-blue-500 text-white' : 'border-gray-300 bg-white text-gray-700' }} font-medium text-sm">
                                Custom Measurements
                            </button>
                        </div>
                    </div>
                ...
                <div class="mt-3 flex gap-2">
                    ...
                    <a href="{{ route('admin.measurements.index') }}" class="flex-1 text-center bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-1 rounded text-sm font-medium">
                        Cancel
                    </a>
                </div>
            </form>
    ...
    <script>
        ...
        function switchToStandardMode() {
            ...
            // Update buttons
            document.getElementById('standardSizeBtn').className = 'size-mode-btn active flex-1 px-3 py-2 rounded-lg border-2 border-blue-500 bg-blue-500 text-white font-medium text-sm';
            document.getElementById('customSizeBtn').className = 'size-mode-btn flex-1 px-3 py-2 rounded-lg border-2 border-gray-300 bg-white text-gray-700 font-medium text-sm';
        ...
        function switchToCustomMode() {
            ...
            // Update buttons
            document.getElementById('customSizeBtn').className = 'size-mode-btn active flex-1 px-3 py-2 rounded-lg border-2 border-blue-500 bg-blue-500 text-white font-medium text-sm';
            document.getElementById('standardSizeBtn').className = 'size-mode-btn flex-1 px-3 py-2 rounded-lg border-2 border-gray-300 bg-white text-gray-700 font-medium text-sm';
